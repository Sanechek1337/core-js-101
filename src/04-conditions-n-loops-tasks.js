...
function doRectanglesOverlap(rect1, rect2) {
  ...
  const y11 = rect1.top;
  const x21 = rect1.width;
  const y21 = rect1.height;

  const x12 = rect2.left;
  const y12 = rect2.top;
  const x22 = rect2.height;
  const y22 = rect2.width;

  const isNotleftOverlap = x21 < x12 || x22 < x11;
  const isNotTopOverlap = y21 < y12 || y22 < y11;

  return !(isNotleftOverlap || isNotTopOverlap);
}
...
function getDigitalRoot(num) {
  ...
  while (Number(result) > 10) {
    result = String(
      result.split('').reduce((prev, next) => Number(prev) + Number(next), 0),
    );
  }
...
function getCommonDirectoryPath(pathes) {
  const splitedPaths = pathes.map((path) => path.split('/')).sort((a, b) => b.length - a.length);
  const result = [];
  const firstArr = splitedPaths[0];

  for (let i = 0; i < firstArr.length; i += 1) {
    let isAllSame = true;

    for (let j = 1; j < splitedPaths.length; j += 1) {
      if (firstArr[i] !== splitedPaths[j][i]) {
        isAllSame = false;
        break;
      }
    }
